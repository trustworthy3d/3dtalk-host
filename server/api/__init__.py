...
#add ky keivn, for set system's time
...
def performSystemAction():
	...
	if request.values.has_key("action"):
		...
		availableActions = s().get(["system", "actions", s().get(["appearance", "language"])])
...
@api.route("/system/time", methods=["GET", "POST"])
@restricted_access
def setSystemTime():
	if request.method == "GET":
		try: 
			utcMilliSecs = float(systemTimeMgt().getNetworkTime())
			return jsonify({"NetworkTime": utcMilliSecs})
		except: pass
	elif request.method == "POST":
		if not "application/json" in request.headers["Content-Type"]:
			return make_response("Expected content type JSON", 400)
		data = request.json
		try: systemTimeMgt().setSystemTime(data["utcMilliSecs"])
		except: pass
	return NO_CONTENT
#add end
		...
		if user is not None and not user.is_anonymous():
			...
			try:
				...
				if netaddr.IPAddress(remoteAddr) in localNetworks \
				   or s().getBoolean(["accessControl", "autologinWeb"]) is True: #modify by kevin, for auto login web
					if netaddr.IPAddress(remoteAddr) in localNetworks:
						autologinAs = s().getBoolean(["accessControl", "defaultUsers", "admin", "username"])
					user = octoprint.server.userManager.findUser(autologinAs)
					if user is not None:
						login_user(user, remember=True) #modify by kevin, for autoLogin
						identity_changed.send(current_app._get_current_object(), identity=Identity(user.get_id()))
						return jsonify(user.asDict())
			except:
				logger = logging.getLogger(__name__)
...
def logout():
	...
	for key in ('identity.id', 'identity.auth_type'):
		try: del session[key] #modify by kevin, for autoLogin
