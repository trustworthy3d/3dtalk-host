...
from octoprint.server import printer, restricted_access, NO_CONTENT, admin_permission, getLocalIp
...
@api.route("/printer/system/onOffStream", methods=["POST"])
@restricted_access
...
def onOffStream():
	...
	if "enabled" in data.keys():
		if data["enabled"] is True:
			...
			streamUrl = streamTmp = settings().get(["webcam", "stream"])
			if wlan0_ip is not None:
				streamUrl = re.sub(r"(//\d+\.\d+\.\d+\.\d+:|//\w+:)", "//" + str(wlan0_ip) + ":", streamTmp)
			else:
				streamUrl = settings().get(["webcam", "streamBk", "stream"])
			
			settings().set(["webcam", "stream"], streamUrl)
			settings().set(["webcam", "streamBk", "flag"], False)
			#subprocess.check_output("/home/pi/scripts/start-stop-webcamDaemon start", shell=True)
			...
			#subprocess.check_output("/home/pi/scripts/start-stop-webcamDaemon stop", shell=True)
...
#add by kevin, for read or write serial number
@api.route("/printer/command/sn", methods=["GET"])
@restricted_access
def getSerialNumber():
	if printer.isClosedOrError() or printer.isError():
		return jsonify(serialNumber="TW00-00000000-000")
	else:
		return jsonify(serialNumber=settings().get(["printerParameters", "serialNumber"]))
#add end
