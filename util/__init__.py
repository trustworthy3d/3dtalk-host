...
#add by kevin, for check some scripts
def check_config():
	try:
		path = glob.glob("/home/pi/oprint/lib/python2.7/site-packages/OctoPrint*/octoprint/util/scripts")[0]
		rpi_gpio = "/home/pi/oprint/bin/rpi_gpio"
		if not os.path.isfile(rpi_gpio):
			py = os.path.join(path, "rpi_gpio.py")
			shutil.copy(py, rpi_gpio)
		if not os.access(rpi_gpio, os.X_OK):
			os.chmod(rpi_gpio, 0754)
		
		config_octo = "/home/pi/oprint/bin/config_octo"
		if not os.path.isfile(config_octo):
			sh = os.path.join(path, "config_octo.sh")
			shutil.copy(sh, config_octo)
		if not os.access(config_octo, os.X_OK):
			os.chmod(config_octo, 0754)
	except: pass
