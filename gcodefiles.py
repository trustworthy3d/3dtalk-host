...
class GcodeManager:
	def __init__(self):
		...
		#add by kevin, for changeFilesPath
		self._usbpath = self._setUsbFolder("udisk")
		self._localpath = self._settings.getBaseFolder("uploads")
		#add end
		self._updates = self._settings.getBaseFolder("updates")

		self._copyPercent = 0 #add by kevin, for copyFile
	...
	#add by kevin, for multi platform
	def _setUsbFolder(self, dirname):
		if sys.platform == "darwin":
			USBDIR = os.path.join(NSSearchPathForDirectoriesInDomains(14, 1, True)[0], dirname.lower())
		elif sys.platform == "win32":
			USBDIR = os.path.join(os.environ["APPDATA"], dirname.lower())
		else:
			USBDIR = os.path.expanduser(os.path.join("~", dirname.lower()))
	...
	def _onMetadataAnalysisFinished(self, filename, gcode):
		...
		if gcode.extrusionAmount:
			...
			for i in range(len(gcode.extrusionAmount)):
				...
				#modify by kevin, for checkout error gcodefiles
				if 0.0 < gcode.extrusionAmount[i] and 0.0 < gcode.extrusionVolume[i]:
					dirty = True
		...
		if dirty:
			...
			os.system('rm %s -rf' %absolutePath)
			self._logger.info("[%s] is not a correct gcode file,so it has been removed!" %basename)
			#modify end, checkout
	...
	def addFile(self, file, destination, uploadCallback=None):
		...
		#modify by kevin, for only upload file to local
		#absolutePath = self.getAbsolutePath(filename, mustExist=False)
		absolutePath = os.path.join(self._localpath, secure_filename(filename))
		#modify end
		...
		#add by kevin, for rename the same name file
		# filename = self._getBasicFilename(absolutePath)
		# filename = self.judgeFileIsExistAndRename(filename, self._uploadFolder, "upload")
		# absolutePath = os.path.join(self._uploadFolder, filename)
		#add end
	...
	#add by kevin, for save our updated progress file
	def addUpdateFile(self, file, uploadCallback=None):
		if not file: return None, True

		filename = file.filename
		# TODO: detect which type of file and add in the extra folder portion 
		secure_absolutePath = os.path.join(self._updates, secure_filename(filename))

		file.save(secure_absolutePath)
	...
	def judgeFileIsExistAndRename(self, filename, path, rename=None):
		def judgeFileIsExist(filename, path):
			files = self.getAllFilenames(path)
			return filename in files
			
		if rename is None:
			return judgeFileIsExist(filename, path)
		elif rename in ("copy", "upload"):
			count = 0
			orgname = filename
			while judgeFileIsExist(filename, path):
				count += 1
				filename = rename + str(count) + "_" + orgname
			if count:
				self._logger.info("[%s] is exist,so it has been rename of [%s]!" %(orgname, filename))
			return filename
	
	def startThreadToCopyFile(self, filename, copyCallback=None, timeout=0):
		srcfile = self.getAbsolutePath(filename)
		if self._isCopying or srcfile is None:
			return
		
		if self._uploadFolder == self._localpath: #add by kevin, for getAbsolutePath
			self._uploadFolder = self._usbpath
		elif self._uploadFolder == self._usbpath:
			self._uploadFolder = self._localpath
		
		filename = self.judgeFileIsExistAndRename(filename, self._uploadFolder, "copy")
		dstfile = os.path.join(self._uploadFolder, filename)
		
		if self._uploadFolder == self._localpath:
			self._uploadFolder = self._usbpath
		elif self._uploadFolder == self._usbpath:
			self._uploadFolder = self._localpath
		
		copyer = threading.Thread(target=self.copyFile,args=(dstfile, srcfile, copyCallback))
		copyer.daemon = True
		copyer.start()
		copyer.join(timeout) #wait for copyer thread return
		return filename

	
	def copyFile(self, dstFile, srcFile, copyCallback=None):
		...
		curaEnabled = self._settings.getBoolean(["cura", "enabled"])
		gcode = isGcodeFileName(dstFile)
		if dstFile is None or (not curaEnabled and not gcode):
			return None, True
		if gcode:
			return self.processGcode(dstFile, FileDestinations.LOCAL, copyCallback), True
		else:
			if curaEnabled and isSTLFileName(dstFile):
				return self.processStl(dstFile, FileDestinations.LOCAL, copyCallback), False
			else:
				return dstFile, False
		#modify end
	#add end, copyFile
	...
	#modify by kevin, for can get alldata come from different path
	def getAllFilenames(self, path):
		return map(lambda x: x["name"], self.getAllFileData(path))
...
class MetadataAnalyzer:
	def __init__(self, getPathCallback, loadedCallback):
		...
		#modify by kevin, for do not analysis gcode
