function GcodeViewModel(loginStateViewModel, settingsViewModel) {
    ...
    self.ui_progress_text = ko.computed(function() {
        ...
        switch (self.ui_progress_type()) {
            case "loading": {
                text = "正在加載... (" + self.ui_progress_percentage().toFixed(0) + "%)";
                break;
            }
            case "analyzing": {
    ...
    self._onModelLoaded = function(model) {
        if (!model) {
            ...
            if (model.filament.length == 0) {
                output.push("總的耗材用量：" + model.filament.toFixed(2) + "mm");
            } else {
                for (var i = 0; i < model.filament.length; i++) {
                    output.push("其中，（擠出頭 " + i + "）用量：" + model.filament[i].toFixed(2) + "mm");
                }
            }
            output.push("預估列印時間：" + formatDuration(model.printTime));
            output.push("預估列印層厚：" + model.layerHeight.toFixed(2) + "mm");
            output.push("預估列印層數：" + model.layersPrinted.toFixed(0) + " 層被列印, " + model.layersTotal.toFixed(0) + " 層被選擇");
    ...
    self._onLayerSelected = function(layer) {
        if (!layer) {
            ...
            var output = [];
            output.push("列印層數計數：" + layer.number);
            output.push("實際列印層厚（毫米）：" + layer.height);
            output.push("當前層G代碼總數：" + layer.commands);
            if (layer.filament.length == 1) {
                output.push("當前層耗材用量：" + layer.filament[0].toFixed(2) + "毫米");
            } else {
                for (var i = 0; i < layer.filament.length; i++) {
                    output.push("其中，（擠出頭 " + i + "）用量：" + layer.filament[i].toFixed(2) + "毫米");
                }
            }
            output.push("當前層列印時間：" + formatDuration(layer.printTime));
