function ControlViewModel(loginStateViewModel, settingsViewModel) {
	...
	self.toolNum = ko.observable("擠出頭 0"); //add by kevin, for display toolNum
    ...
    self.settings.printer_numExtruders.subscribe(function(oldVal, newVal) {
        ...
        if (numExtruders > 1) {
            ...
            for (var extruder = 0; extruder < numExtruders; extruder++) {
                ...
                tools[extruder]["name"]("擠出頭 " + extruder);
	...
	self.onOffStream = function(data) {
		...
		var showConfirmationDialog = function() {
			...
			if(CONFIG_WEBCAM_STREAM == "http://your_printer_ip:8080/?action=stream"){
				$(".confirmation_dialog_message", confirmationDialog).text("您確定要打開即時視頻嗎？這可能會對性能造成負面影響，不建議長時打開。如果打開之後，它將在一小時後自動關閉。");
			}else{
				$(".confirmation_dialog_message", confirmationDialog).text("您確定要立即關閉即時視頻嗎？");
			}
