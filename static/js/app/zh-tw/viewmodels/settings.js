function SettingsViewModel(loginStateViewModel, usersViewModel) {
    ...
    self.removeUpdateFile = function(filename) {
        if (self.isUpdating() == true) {
            $.pnotify({title: "警告", text: "<p>正在進行更新操作，請等待完成後重試！</p>", type: "error"});
            return;
        }
        $.ajax({
    ...
    self.uploadUpdateFile = function(filename) {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                self.finishUpdate();
                $.pnotify({title: "錯誤", text: "<p>\"" + filename + "\" 的更新操作執行失敗！</p><p>原因是：<pre>" + jqXHR.responseText + "</pre></p>", type: "error"});
            }
        });
    }
    ...
    function update_upload_fail(e, data) {
        $.pnotify({
            title: "上傳失敗",
            text: "<p>不能上傳這個檔。請確認它是一個帶有\".zip\"、\".hex\"或\".3dt\"尾碼副檔名的文件。</p><p>伺服器的報告是: <pre>" + data.jqXHR.responseText + "</pre></p>",
            type: "error",
            hide: false
        });
        $("#update_upload_progress .bar").css("width", "0%");
    ...
    function update_upload_progress(e, data) {
        ...
        $("#update_upload_progress .bar").text("正在上傳 ...");
        if (progress >= 100) {
