function TemperatureViewModel(loginStateViewModel, settingsViewModel) {
    ...
    self._numExtrudersUpdated = function() {
        ...
        if (numExtruders && numExtruders > 1) {
            ...
            for (var extruder = 0; extruder < numExtruders; extruder++) {
				...
				if (CONFIG_HEATEDCHAMBER && extruder == numExtruders-1){
					heaterOptions["tool" + extruder] = {name: "打印仓", color: color};
				}else{
					// modi end, chamber
					heaterOptions["tool" + extruder] = {name: "挤出头 T" + extruder, color: color};
				}
				...
				//modify by kevin, for use tool2 replace chamber
				if (CONFIG_HEATEDCHAMBER && extruder == numExtruders-1){
					tools[extruder]["name"]("打印仓");
					tools[extruder]["key"]("tool" + 2);
				}else{
					// modi end, chamber
					tools[extruder]["name"]("挤出头 T" + extruder);
        ...
        // print bed
        heaterOptions["bed"] = {name: "打印床", color: "blue"};
    ...
    self.updatePlot = function() {
        ...
        if (graph.length) {
            ...
            _.each(_.keys(heaterOptions), function(type) {
                ...
                data.push({
                    label: "" + heaterOptions[type].name + " 实际温度：" + actualTemp,
                    color: heaterOptions[type].color,
                    data: actuals
                });
                data.push({
                    label: "" + heaterOptions[type].name + " 目标温度：" + targetTemp,
