function SettingsViewModel(loginStateViewModel, usersViewModel) {
    ...
    self.appearance_available_languages = ko.observable([]); //add by kevin,for multiLanguage
    ...
    self.printer_numExtruders = ko.observable([]);
...
/*    self.showWifiPassword = function() {
    }*/
    ...
    self.removeUpdateFile = function(filename) {
        if (self.isUpdating() == true) {
            $.pnotify({title: "警告", text: "<p>正在进行更新操作，请等待完成后重试！</p>", type: "error"});
            return;
        }
        $.ajax({
    ...
    self.uploadUpdateFile = function(filename) {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                self.finishUpdate();
                $.pnotify({title: "错误", text: "<p>\"" + filename + "\" 的更新操作执行失败！</p><p>原因是：<pre>" + jqXHR.responseText + "</pre></p>", type: "error"});
            }
        });
    }
    ...
    self.getProgress = function() {
        ...
        $("#update_upload_progress .bar").text("正在更新 ...");
    ...
    function update_upload_fail(e, data) {
        $.pnotify({
            title: "上传失败",
            text: "<p>不能上传这个文件。请确认它是一个带有\".zip\"、\".hex\"或\".3dt\"后缀扩展名的文件。</p><p>服务器的报告是: <pre>" + data.jqXHR.responseText + "</pre></p>",
            type: "error",
            hide: false
        });
        $("#update_upload_progress .bar").css("width", "0%");
    ...
    function update_upload_progress(e, data) {
        ...
        $("#update_upload_progress .bar").text("正在上传 ...");
        if (progress >= 100) {
            $("#update_upload_progress").addClass("progress-striped").addClass("active");
            $("#update_upload_progress .bar").text("正在保存 ...");
        }
    }
