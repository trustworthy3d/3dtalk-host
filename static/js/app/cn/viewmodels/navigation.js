function NavigationViewModel(loginStateViewModel, appearanceViewModel, settingsViewModel, usersViewModel) {
    ...
    self.triggerLanguage = function(languages) {
        if(self.recordlast_language() != languages.language){
            var data = {"appearance" : {"language": languages.language} };
            $.ajax({
    ...
    self.triggerAction = function(action) {
        var callback = function() {
            $.ajax({
                ...
                success: function() {
                    $.pnotify({title: "成功", text: "命令 \""+ action.name +"\" 执行成功", type: "success"});
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    $.pnotify({title: "错误", text: "<p>命令 \"" + action.name + "\" 不能被执行。</p><p>原因是：<pre>" + jqXHR.responseText + "</pre></p>", type: "error"});
                }
            })
        }
        if (action.confirm) {
	...
	self.networkActions = [
		{"action": "connect", "name": "连接WiFi"},
		{"action": "create", "name": "创建WiFi"}
	];
