function GcodeViewModel(loginStateViewModel, settingsViewModel) {
    ...
    self.ui_progress_text = ko.computed(function() {
        ...
        switch (self.ui_progress_type()) {
            case "loading": {
                text = "正在加载... (" + self.ui_progress_percentage().toFixed(0) + "%)";
                break;
            }
            case "analyzing": {
                text = "正在分析... (" + self.ui_progress_percentage().toFixed(0) + "%)";
                break;
            }
            case "done": {
                text = "分析完成";
    ...
    self._onModelLoaded = function(model) {
        if (!model) {
            ...
            var output = [];
            output.push("模型尺寸：" + model.width.toFixed(2) + "mm &times; " + model.depth.toFixed(2) + "mm &times; " + model.height.toFixed(2) + "mm");
            if (model.filament.length == 0) {
                output.push("总的耗材用量：" + model.filament.toFixed(2) + "mm");
            } else {
                for (var i = 0; i < model.filament.length; i++) {
                    output.push("其中，（挤出头 " + i + "）用量：" + model.filament[i].toFixed(2) + "mm");
                }
            }
            output.push("预估打印时间：" + formatDuration(model.printTime));
            output.push("预估打印层厚：" + model.layerHeight.toFixed(2) + "mm");
            output.push("预估打印层数：" + model.layersPrinted.toFixed(0) + " 层被打印, " + model.layersTotal.toFixed(0) + " 层被选择");
    ...
    self._onLayerSelected = function(layer) {
        if (!layer) {
            ...
            var output = [];
            output.push("打印层数计数：" + layer.number);
            output.push("实际打印层厚（毫米）：" + layer.height);
            output.push("当前层G代码总数：" + layer.commands);
            if (layer.filament.length == 1) {
                output.push("当前层耗材用量：" + layer.filament[0].toFixed(2) + "毫米");
            } else {
                for (var i = 0; i < layer.filament.length; i++) {
                    output.push("其中，（挤出头 " + i + "）用量：" + layer.filament[i].toFixed(2) + "毫米");
                }
            }
            output.push("当前层打印时间：" + formatDuration(layer.printTime));
