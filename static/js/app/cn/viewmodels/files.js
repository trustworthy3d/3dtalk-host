function GcodeFilesViewModel(printerStateViewModel, loginStateViewModel) {
	...
	self.getProgress = function() {
        ...
        $("#gcode_upload_progress .bar").text("正在拷贝 ...");
    ...
    self.getPopoverContent = function(data) {
        var output = "<p><strong>上传时间：</strong> " + formatDate(data["date"]) + "</p>";
        if (data["gcodeAnalysis"]) {
            ...
            output += "<strong>预估打印时间：</strong> " + formatDuration(data["gcodeAnalysis"]["estimatedPrintTime"]);
            output += "</p>";
        }
        if (data["prints"] && data["prints"]["last"]) {
            output += "<p>";
            output += "<strong>上次打印时间：</strong> <span class=\"" + (data["prints"]["last"]["success"] ? "text-success" : "text-error") + "\">" + formatDate(data["prints"]["last"]["date"]) + "</span>";
            output += "</p>";
        }
        return output;
    };
