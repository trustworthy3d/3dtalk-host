$(function() {
        ...
        function gcode_upload_fail(e, data) {
            $.pnotify({
                title: "上传失败",
                text: "<p>不能上传这个文件。请确认它是一个带有\".gcode\"、\".gco\"或\".g\"后缀扩展名的GCODE文件，或者您是在您的Cura切片软件被配置且开启的前提下，上传的一个带有\".stl\"后缀扩展名的STL文件。</p><p>服务器的报告是: <pre>" + data.jqXHR.responseText + "</pre></p>",
                type: "error",
                hide: false
            });
            $("#gcode_upload_progress .bar").css("width", "0%");
        ...
        function gcode_upload_progress(e, data) {
            ...
            $("#gcode_upload_progress .bar").text("正在上传 ...");
            if (progress >= 100) {
                $("#gcode_upload_progress").addClass("progress-striped").addClass("active");
                $("#gcode_upload_progress .bar").text("正在保存 ...");
            }
        }
		...
		gcodeFilesViewModel.isEnUpload.subscribe(function(newValue) {
			if (newValue === true) {
				enable_local_dropzone();
			} else {
				disable_local_dropzone();
        ...
        //add by kevin
        ko.applyBindings(navigationViewModel, document.getElementById("network-otherDialogConnectWifi"));
        ko.applyBindings(navigationViewModel, document.getElementById("network-otherDialogCreateWifi"));
        //add end
		...
		//add by kevin, for set system's time
		if (CONFIG_POST_TIME) {
			$.ajax({
				url: API_BASEURL + "system/time",
				type: "POST",
				dataType: "json",
				contentType: "application/json; charset=UTF-8",
				data: JSON.stringify({utcMilliSecs: Date.now()})
			})
		}
		//add end
    }
