function UsersViewModel(loginStateViewModel) {
    ...
    self.removeUser = function(user, callback) {
		...
		if (user.name=="root" || user.name=="ouring") {
			$.pnotify({title: "Not possible", text: "You are not allowed to modify the system built-in accounts", type: "error"});
			return;
		}
		//add end
    ...
    self.updatePassword = function(username, password, callback) {
		...
		//add by kevin, for manage default users
		if (username=="root" || username=="ouring") {
			$.pnotify({title: "Not possible", text: "You are not allowed to modify the system built-in accounts", type: "error"});
			return;
		}
		//add end
