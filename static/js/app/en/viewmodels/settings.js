function SettingsViewModel(loginStateViewModel, usersViewModel) {
    ...
    self.appearance_language = ko.observable(undefined); //add by kevin,for multiLanguage
    self.recordlast_language = self.appearance_language;

    self.appearance_available_colors = ko.observable(["default", "red", "orange", "yellow", "green", "blue", "violet", "black"]);
    self.appearance_available_languages = ko.observable(["chinese", "english"]); //add by kevin,for multiLanguage
    ...
    self.getAvailableLanguages = function(data) {
        var languages = [];
        ...
        if(languages.length){
            self.appearance_available_languages(languages);
        }else{
            self.appearance_available_languages(["chinese", "english", "german"]);
        }
    }
		...
		$.ajax({
            ...
            success: function(response) {
                if(response.wifiNames.length){
					...
					self.availableWifiNames(["Searching..."]);
				}
            }
        });
	}
	...
	self.showConnectWifiDialog = function() {
		$("#settings-otherDialogConnectWifi").modal("show");
	}
	...
	self.confirmConnectWifi = function() {
		...
		$("#settings-otherDialogConnectWifi").modal("hide");
	}
	
	self.showCreateWifiDialog = function() {
		$("#settings-otherDialogCreateWifi").modal("show");
	}
	...
	self.confirmCreateWifi = function() {
		...
		$("#settings-otherDialogCreateWifi").modal("hide");
	}
	...
	self.showFactoryResetDialog = function() {
		$("#settings-otherDialogFactoryReset").modal("show");
	}
	
	self.confirmFactoryReset = function() {
		self.systemCommand();
		$("#settings-otherDialogFactoryReset").modal("hide");
	}
	//add end, for some other settings

    //add by kevin, for update our progress
    self.timer = null;
    self.updatedPercent = 0;
    self.isUpdating = ko.observable(false);
    self.availableUpdateFiles = ko.observableArray([]);
    self.requestUpdateFiles = function() {
        $.ajax({
            url: API_BASEURL + "getUpdateFiles",
            type: "GET",
            dataType: "json",
            success: function(response) {
                self.availableUpdateFiles(response.files);
            }
        });
    };

    self.removeUpdateFile = function(filename) {
        if (self.isUpdating() == true) {
            $.pnotify({title: "WARN", text: "<p>After completion of ongoing updates, please wait try again!</p>", type: "error"});
            return;
        }
        $.ajax({
            url: API_BASEURL + "updates/" + filename,
            type: "DELETE",
            dataType: "json",
            success: self.requestUpdateFiles
        });
    }

    self.uploadUpdateFile = function(filename) {
        if (self.isUpdating() == true) {
            $.pnotify({title: "WARN", text: "<p>After completion of ongoing updates, please wait try again!</p>", type: "error"});
            return;
        }
        $.ajax({
            ...
            success: function () {
                ...
                self.getProgress();
                self.requestUpdateFiles();
            },
            error: function(jqXHR, textStatus, errorThrown) {
                self.finishUpdate();
                $.pnotify({title: "ERROR", text: "<p>\"" + filename + "\" update failed！</p><p>Reason: <pre>" + jqXHR.responseText + "</pre></p>", type: "error"});
            }
        });
    }

    self.finishUpdate = function() {
        $("#update_upload_progress .bar").css("width", 0 + "%");
        $("#update_upload_progress").removeClass("progress-striped").removeClass("active");
        $("#update_upload_progress .bar").text("");
    ...
    self.getProgress = function() {
        $.ajax({
            url: API_BASEURL + "updateProgress",
            method: "GET",
            dataType: "json",
            success: function(response) {
                self.updatedPercent = response.updatedPercent;
            }
        });

        $("#update_upload_progress .bar").css("width", self.updatedPercent + "%");
        $("#update_upload_progress .bar").text("Updating ...");

        if(100 <= self.updatedPercent) {
            self.timer = setTimeout(self.finishUpdate, 1000);
        } else {
            self.timer = setTimeout(self.getProgress, 100);
        }
    }

    function update_upload_done(e, data) {
        self.availableUpdateFiles(data.result.files);
    ...
    function update_upload_fail(e, data) {
        $.pnotify({
            title: "Upload failed",
            text: "<p>Could not upload the file. Make sure that it has the extension \".zip\"、\".hex\"或\".3dt\"</p><p>Server reported: <pre>" + data.jqXHR.responseText + "</pre></p>",
            type: "error",
            hide: false
        });
        $("#update_upload_progress .bar").css("width", "0%");
    ...
    function update_upload_progress(e, data) {
        var progress = parseInt(data.loaded / data.total * 100, 10);
        $("#update_upload_progress .bar").css("width", progress + "%");
        $("#update_upload_progress .bar").text("Uploading ...");
        if (progress >= 100) {
            $("#update_upload_progress").addClass("progress-striped").addClass("active");
            $("#update_upload_progress .bar").text("Saving ...");
        }
    }

    $("#updateFile_upload").fileupload({
        url: API_BASEURL + "uploadUpdateFile",
        dataType: "json",
        dropZone: null,
        done: update_upload_done,
        fail: update_upload_fail,
        progressall: update_upload_progress
    });
    //add end
	...
	self.webcam_stream = ko.observable(undefined); //add by keivn, for default disable webcamStream
    ...
    self.fromResponse = function(response) {
        ...
        self.appearance_language(response.appearance.language); //add by kevin,for multiLanguage
        self.recordlast_language = self.appearance_language();
        self.getAvailableLanguages(response.appearance.languages);
		...
		self.webcam_stream(response.webcam.alwaysEnableStream); //add by keivn, for default disable webcamStream
    ...
    self.saveData = function() {
        var data = {
            ...
            "appearance" : {
                ...
                "language": self.appearance_language() //add by kevin,for multiLanguage
            },
            "printer": {
            ...
            "webcam": {
                "alwaysEnableStream": self.webcam_stream(), //add by keivn, for default disable webcamStream
		...
		//modify by kevin, for if language has been changed then reload index
		if(self.recordlast_language != self.appearance_language()){
			//alert("last="+self.recordlast_language+"<->cur="+self.appearance_language());
