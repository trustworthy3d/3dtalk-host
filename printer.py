...
import os
import logging
import subprocess #add by kevin, for pump
...
class Printer():
	def __init__(self, gcodeManager):
		...
		try:
			util.check_config()
			subprocess.check_output("sudo ~/oprint/bin/rpi_gpio", shell=True)
		except: pass
	...
	def commands(self, commands):
		...
		#add by kevin, for pump
		if len(commands) and commands[0] in ("PumpOn", "PumpOff"):
			try:
				if commands[0] == "PumpOn":
					subprocess.check_output("sudo ~/oprint/bin/rpi_gpio 1", shell=True)
				elif commands[0] == "PumpOff":
					subprocess.check_output("sudo ~/oprint/bin/rpi_gpio 0", shell=True)
			except: pass
			return
		#add end

		for command in commands:
			self._comm.sendCommand(command.upper()) #modify by kevin

	def jog(self, axis, amount):
		movementSpeed = settings().get(["printerParameters", "movementSpeed", ["x", "y", "z"]], asdict=True)
		moveRange = settings().get(["printerParameters", "moveRange", axis])
		axisCurPos = self._axisCurPos[axis]
	...
	def changeTool(self, tool):
		try:
			...
			#add by kevin
			if toolNum != settings().get(["printerParameters", "toolInfo"])[0]:
				settings().set(["printerParameters", "toolInfo"], [toolNum, True])
			#add end
		except ValueError:
			pass

	#modify by kevin, for when pause or stop ,to go down platform
	def setTemperature(self, type, value, waitFor=False):
		if type.startswith("tool"):
			if settings().getInt(["printerParameters", "numExtruders"]) > 1:
				try:
					toolNum = int(type[len("tool"):])
					self.command("%s T%d S%f" % ("M109" if waitFor else "M104", toolNum, value))
				except ValueError:
					pass
			else:
				self.command("%s S%f" %("M109" if waitFor else "M104", value))
		elif type == "bed":
